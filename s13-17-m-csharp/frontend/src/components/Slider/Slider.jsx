        ...
        <div className="flex transition-transform duration-500 ease-in-out transform">
          {slides.map((slide, index) => (
              ...
              <img
                ...
                src={slide}
                alt={`Slide ${index + 1}`}
                className={`w-full  object-cover object-center ${styles.imagen}  ${index === currentIndex ? 'block' : 'hidden'}`}
              />

{index === 0 && (
                <div className={`${styles.contenedorP} absolute  ml-2 top-0 left-0 p-4 mt-2 bg-gray-200 bg-opacity-70 z-10`}>
                  <p className={`${styles.parrafo}  text-gray-800`} >Registrate en MediCita y agendá tu próximo turno médico desde la comodidad de tu casa</p>
                </div>
              )}
      ...
      <div className="flex items-center justify-center gap-6 -mt-14">
        {slides.map((_, index) => (
          <button
            key={index}
            className={`w-2 h-2 mx-1 rounded-full ${index === currentIndex ? 'bg-[#097DCA] h-[28px] w-[28px]  ' : 'bg-gray-300'}`}
